...
If multiple tiebreakers are given, the first one is applied first. If it is able to rank one entry higher than the other, then this rank is chosen and the next set of result entries are processed. But if the first tiebreaker isn't able to rank the entries, the next tiebreaker in the list is applied and so on.
