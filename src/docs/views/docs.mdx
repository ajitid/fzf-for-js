...
- `algo` - string or `null`, defaults to `"v2"`, accepts `"v1" | "v2" | null` - Select which version of fuzzy algo you want to use. Refer to [this explanation](https://github.com/junegunn/fzf/blob/4c9cab3f8ae7b55f7124d7c3cf7ac6b4cc3db210/src/algo/algo.go#L5) to see the differences. If asssigned `null`, an exact match will be made instead of a fuzzy one.
...
- `forward` - boolean, defaults to `true` - If false, items will be matched from backwards. So for example, "/breeds/pyrenees" when queried with "re" will result in "/b<b>re</b>eds/pyrenees" but with forward=false it will result in "/breeds/py<b>re</b>nees". Turning forward false is useful, for example, when one needs to match a file path and they prefer matching file name over the directory names it is in.
- `sort` - object or `null`, defaults to an object that defines sorting result entries in descending order by score - It can be:
  - null - to disable sorting, or
  - object
    - order - can be `"asc"` or `"desc"` - ascending or descending respectively
    - on - a function that takes in a result entry and returns a number that will be used for comparison
