...
- `forward` - boolean, defaults to `true` - If false, items will be matched from backwards. So for example, "/breeds/pyrenees" when queried with "re" will result in "/b**re**eds/pyrenees" but with forward=false it will result in "/breeds/py**re**nees". Turning forward=false is useful, for example, when one needs to match a file path and they prefer matching file name over the directory names it is in.
