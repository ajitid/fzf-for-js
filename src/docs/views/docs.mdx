...
If there are multiple tiebreakers, the first one is applied first (when the scores are equal), if this ties (i.e. the tiebreaker function returns 0) the second one is applied, etc.
Finally if the scores are equal and all tiebreakers also tie, then the results are returned in the order that they were in the original input list.
...
- `sort?` - boolean, defaults to `true` - If true, result items will be sorted in descending order by their score. Otherwise, result won't be sorted and tiebreakers won't be applied either; they will be returned in the same order that they were in the input list.
