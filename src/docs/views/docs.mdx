...
You would change your code from:
...
If you are typing in query input, and the result for previous query isn't resolved before the next one is send off to FZF, FZF would throw an error for the previous query. So in the case when you've typed "he" in you query and you press "y" to make it "hey", if FZF hasn't send you the result back for "he" yet, it'll throw an error for "he" instead. This is why we need to give an empty function to `catch` if we are not planning to handle it in any manner.
