...
- `tiebreakers` - array, defaults to `[]` - A list of functions that decide sort order when the score is tied between two results. A tiebreaker is nothing but a compare function like the one you find in [JS Array sort](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/sort) which also has a third argument passed which is `selector`. First and second arguments are result entries that you receive from `fzf.find`.
