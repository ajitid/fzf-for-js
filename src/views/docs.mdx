...
<span className="docs-hidden"><i>If you are reading this file on GitHub, we'll suggest to read it on <a href="https://fzf.vercel.app">documentation website</a> instead.</i></span>
...
- `normalize?` - boolean, defaults to `false` - If true, FZF will try to remove diacritics from list items. This is useful if the list contains items with diacritics but you want to query with plain A-Z letters. Example Zoë will be converted to Zoe.
